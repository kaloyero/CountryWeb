...
public class TipoPagosController {
	...
	@Autowired
	private TypePaymentManager typePaymentManager;
	
	@RequestMapping(value = "/create",method = RequestMethod.GET)
	public String showForm(ModelMap model) {
		TipoForm tipo = new TipoForm();
		model.addAttribute("TIPO", tipo);
		model.addAttribute("accion", "tipoPago");
		return "tipoPago";
	}
	...
	public String processForm(
		...
		typePaymentManager.save(TipoMapper.getTipoPagos(tipoForm));
				return "success";
	...
	public String load(ModelMap model,@PathVariable int id) throws ParseException {
		...
		TipoPagos type =typePaymentManager.findById(id);
	...
	public String update(@ModelAttribute(value = "TIPO") TipoForm tipoForm,@PathVariable int id,
			BindingResult result) throws ParseException {
		typePaymentManager.update(TipoMapper.getTipoPagos(tipoForm));
		return "success";
	...
	public  @ResponseBody DataTable getUserInJSON()  {
           ...
           DataTable dataTable=new DataTable();
			...
			for (TipoPagos tipo : typePaymentManager.listAll()) {
